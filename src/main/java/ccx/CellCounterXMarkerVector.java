 ...
 * @author Kurt De Vos
 */
public class CellCounterXMarkerVector extends Vector{
    private int type;
    private Color color;
    /** Creates a new instance of MarkerVector */
    public CellCounterXMarkerVector(int type) {
        super();
        this.type=type;
        color = createColor(type);
    }
    public void addMarker(CellCounterXMarker marker){
        add(marker);
    }
    
    public CellCounterXMarker getMarker(int n){
        return (CellCounterXMarker)get(n);
    }
    public int getVectorIndex(CellCounterXMarker marker){
    ...
    private boolean isCloser(CellCounterXMarker m1,CellCounterXMarker m2, Point p){
    ...
    public CellCounterXMarker getMarkerFromPosition(Point p, int sliceIndex){
        Vector v = new Vector();
        ListIterator it = this.listIterator();
        while(it.hasNext()){
            CellCounterXMarker m = (CellCounterXMarker)it.next();
            if (m.getZ()==sliceIndex){
                v.add(m);
            }
        }
        CellCounterXMarker currentsmallest = (CellCounterXMarker)v.get(0);
        for (int i=1; i<v.size(); i++){
            CellCounterXMarker m2 = (CellCounterXMarker)v.get(i);
